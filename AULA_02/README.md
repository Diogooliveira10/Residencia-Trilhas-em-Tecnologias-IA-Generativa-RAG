...
Este repositório faz parte do Programa de Residência em IA Generativa & RAG da PUC-Rio e contém o código para conversão de arquivos PDF em Markdown utilizando a biblioteca [Docling](https://github.com/docling-project/docling), etapa preparatória para pipelines de RAG (Retrieval-Augmented Generation).
...
├── markdown/                   # Arquivos .md gerados (saída)
...
Com o ambiente ativado, instale as bibliotecas necessárias (como `docling`) a partir do arquivo `requirements.txt` que está na raiz do projeto:
...
Coloque os arquivos PDF que deseja converter dentro da pasta `pdfs/`, na raiz do projeto.
...
Por padrão, o script roda com `do_ocr=False`, pois os PDFs utilizados possuem texto nativo (não são digitalizados/escaneados). Isso evita erros de estouro de memória (`std::bad_alloc`) que podem ocorrer ao processar imagens em alta resolução durante o OCR. Caso precise converter PDFs escaneados, ative o OCR alterando essa opção no script.
